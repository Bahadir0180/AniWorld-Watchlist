...
app.post('/api/fetch-watchlist', async (req, res) => {
    ...
    try {
        ...
        browser = await chromium.launch({ headless: false, channel: 'msedge', slowMo: 50 });
        ...
        for (const anime of animeList) {
            ...
            // 1. Staffel-Links finden (z.B. Staffel 1, Staffel 2, Filme)
            const subPageLinks = await page.evaluate(() => {
                const links = [];
                // Das erste <ul> in der Navi enthält gewöhnlich die Staffeln
                const seasonUl = document.querySelector('#stream ul:nth-of-type(1)');
                if (seasonUl) {
                    seasonUl.querySelectorAll('li a').forEach(a => {
                        const href = a.getAttribute('href') || '';
                        // Nur Links aufnehmen, die nach Staffeln oder Filmen aussehen, keine JS-Links
            ...
            // Funktion zur Ermittlung NUR echter Episoden auf der aktuellen Unterseite
            const evaluateEpisodesOnly = () => {
                // Das zweite <ul> enthält die Episoden-Buttons
                ...
                // Filtere strikt: Nur Links, die zu einer konkreten Episode gehören (/episode-X oder Zahl als Text)
            ...
            if (subPageLinks.length > 0) {
                ...
                // Fallback, falls der Anime keine Staffel-Navigation hat
                const pageStats = await page.evaluate(evaluateEpisodesOnly);
                totalEpisodesCount = pageStats.total;
                watchedEpisodesCount = pageStats.watched;
...
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'index.html')); // Passe den Pfad zu deiner HTML-Datei an
});
